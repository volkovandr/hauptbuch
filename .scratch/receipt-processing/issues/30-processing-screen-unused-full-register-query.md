...
Fixed 2026-09-19 on `feat/reporting` (commit `c9d76b8`): added `RegisterService.datalists()`,
which assembles the same option lists (`assembleView`, extracted from `view()`) without resolving
the viewed accounts or running `RegisterRepository.findRows`, and pointed
