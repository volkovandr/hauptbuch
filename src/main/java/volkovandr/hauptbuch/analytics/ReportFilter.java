 ...
 * @param level the transaction/posting reading (§6.2); {@link FilterField#ACCOUNT_TYPE} accepts
 *     only {@link FilterLevel#TRANSACTION} — its posting-level reading is exactly what scope's
 *     account types already say, so offering it would be a second control for one meaning (§6.2)
 ...
 *     {@link FilterField#RECONCILIATION}; a single regular expression for {@code MATCHES}; a single
 *     substring for {@code CONTAINS}
 */
public record ReportFilter(
  ...
  /**
   * Defensively copies {@code values}, rejects an empty list, rejects an operator {@code field}
   * does not offer (§6.3), and rejects a posting-level reading of {@link FilterField#ACCOUNT_TYPE}
   * (§6.2) — category errors the UI must never present, checked here so a bad combination fails at
   * spec construction rather than producing a query with no matching SQL branch.
   */
  public ReportFilter {
    ...
    if (field == FilterField.ACCOUNT_TYPE && level != FilterLevel.TRANSACTION) {
      throw new IllegalArgumentException(
          "Account type filters are transaction-level only (§6.2) — scope already expresses the"
              + " posting-level reading.");
    }
