...
--
-- System accounts (plan stage 3): the Opening Balances equity tree and the FX gain/loss tree.
-- Both follow the per-currency-leaf category rule (data-model §6.5): a semantic parent with one
-- leaf per currency, because every account — categories and system accounts included — has exactly
-- one currency. Postings hit the leaves; the parent is a pure rollup (leaves-only, §5).
--
...
-- opening balance, the base-currency FX gain/loss leaf for a conversion residual (both are
-- base-denominated facts) — whatever base currency the book is later locked to.
...
-- ── FX gain/loss (income) — the residual that balances a non-par conversion (data-model §6.3) ──
-- A single signed leaf per currency; the sign carries gain vs loss. Filed under income (T-DM-3,
-- cosmetic); the engine books only the base-currency leaf, at genuine two-currency events.
insert into account (name, type, parent_id, currency_code)
  values ('FX gain/loss', 'income', null, 'EUR');

insert into account (name, type, parent_id, currency_code)
select 'FX gain/loss ' || c.currency_code, 'income', p.account_id, c.currency_code
from currency c
cross join (select account_id from account where name = 'FX gain/loss' and parent_id is null) p;
